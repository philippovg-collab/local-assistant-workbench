...
final class DocumentBlockHeuristics {
    ...
    static DocumentBlockConfidence confidenceFor(boolean ocrUsed) {
        return ocrUsed ? DocumentBlockConfidence.LOW : DocumentBlockConfidence.HIGH;
    }

    static DocumentBlockType classify(String text, DocumentBlockType fallbackType) {
    ...
    static boolean looksLikeTitle(String text) {
        String normalized = normalizeText(text);
    ...
    static boolean looksLikeList(String text) {
        String normalized = normalizeText(text);
        if (!StringUtils.hasText(normalized)) {
            return false;
        }
    ...
    static boolean looksLikeCaption(String text) {
        return CAPTION_PATTERN.matcher(normalizeText(text)).matches();
    }

    static Integer levelFor(DocumentBlockType type, String text) {
