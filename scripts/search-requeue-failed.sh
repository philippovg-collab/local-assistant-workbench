...
if [[ -z "$JAVA21_HOME" ]]; then
  echo "Java 21 not found. Install OpenJDK 21 or export JAVA_21_HOME before running search recovery." >&2
  exit 1
fi
...
mvn -q -DskipTests spring-boot:run \
  "-Dspring-boot.run.arguments=--app.search-sync.operator.command=requeue-failed"
